                            ...
                            float *aim_x, float *aim_y, float *aim_z)
{
    ...
    else if(st.armor_type == 4)
    {
        for (i = 0; i<3; i++) {
            float tmp_yaw = st.tar_yaw + i * 2.0f * PI / 3.0f;
            float r = use_1 ? st.tar_r1 : st.tar_r2;
            tar_position[i].x = xw - r*cos(tmp_yaw);
            tar_position[i].y = yw - r*sin(tmp_yaw);
            tar_position[i].z = use_1 ? zw : st.z2;
            tar_position[i].yaw = st.tar_yaw+ i * 2.0f * PI / 3.0f;
            use_1 = !use_1;
        }
    }
...
//       	}
        ...
        float yaw_diff_min = fabs(*yaw - tar_position[0].yaw);
        for (i = 1; i<4; i++) {
            float temp_yaw_diff = fabs(*yaw - tar_position[i].yaw);
            if (temp_yaw_diff < yaw_diff_min)
            {
                yaw_diff_min = temp_yaw_diff;
                idx = i;
