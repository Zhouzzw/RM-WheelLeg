...
void UI_Task(void const * argument)
{
	...
	while(1)
	{
        ...
        else
        {
            ...
            UI_State.Fric1_Speed = Motor_Data_Struct[Shoot_Fric_Left_ID].SpeedRPM  ;
            UI_State.Fric2_Speed = Motor_Data_Struct[Shoot_Fric_Right_ID].SpeedRPM ;
            UI_State.Fric3_Speed = Motor_Data_Struct[Shoot_Fric_Middle_ID].SpeedRPM;       
            UI_State.Bullet      = Robo_BulletRemain.projectile_allowance_42mm;
            UI_State.SuperCap_IF_Enable = RoboControl_Struct.SuperCap_State;
            UI_State.SuperCap_K = (RoboControl_Struct.SuperCap_V - 6.0f) / 18.0f; //超电安全电压:8~24V
            if(UI_State.SuperCap_K < 0) UI_State.SuperCap_K = 0;
            UI_State.Chassis_Motor1_Speed = Motor_Data_Struct[Chassis_Motor1_ID].SpeedRPM * Chassis_RPM_to_m_s;
            UI_State.Chassis_Motor2_Speed = Motor_Data_Struct[Chassis_Motor2_ID].SpeedRPM * Chassis_RPM_to_m_s;
            UI_State.Chassis_Motor3_Speed = Motor_Data_Struct[Chassis_Motor4_ID].SpeedRPM * Chassis_RPM_to_m_s;
            UI_State.Chassis_Motor4_Speed = Motor_Data_Struct[Chassis_Motor3_ID].SpeedRPM * Chassis_RPM_to_m_s;
